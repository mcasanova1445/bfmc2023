...
STOP_WAIT_TIME = 0.001*3.0 if not SPEED_CHALLENGE else 0.0  # 3.0              ## why this short? 0.003?
# local tracking
OPEN_LOOP_PERCENTAGE_OF_PATH_AHEAD = 0.6  # 0.6
# distance from previous stop_line from which is possible to
# start detecting a stop line again
STOP_LINE_DISTANCE_THRESHOLD = 0.2
...
class Brain:
    def __init__(self,
        ...
        self.states = {
            ...
            nac.APPROACHING_STOP_LINE:
            State(nac.APPROACHING_STOP_LINE, self.approaching_stop_line),
        ...
        self.routines = {
            ...
            nac.DETECT_STOP_LINE:
            Routine(nac.DETECT_STOP_LINE,  self.detect_stop_line),
            nac.SLOW_DOWN:
    ...
    def start_state(self):
        ...
        if self.conditions[nac.TRUST_GPS]:
            if self.car.gps_cnt < 10:
                print("WARNING: Reading non-filtered GPS position")
            # get closest node
    ...
    def lane_following(self):  # LANE FOLLOWING ##############################
        ...
        # we are approaching a stop_line, check only if we are far enough
        # from the previous stop_line
        else:
            if self.conditions[nac.TRUST_GPS]:
                ...
                if self.detect.est_dist_to_stop_line < \
                        STOP_LINE_APPROACH_DISTANCE and \
                        far_enough_from_prev_stop_line and \
    ...
    def approaching_stop_line(self):
        # FOLLOW_LANE, SLOW_DOWN, DETECT_STOP_LINE, CONTROL_FOR_OBSTACLES
        self.activate_routines([nac.FOLLOW_LANE,
                                nac.SLOW_DOWN,
                                nac.DETECT_STOP_LINE,
                                nac.CONTROL_FOR_OBSTACLES])
        ...
        if self.conditions[nac.TRUST_GPS] and not \
            ...
            decide_next_state = self.approaching_stop_line_gps()
        else:
            decide_next_state = self.approaching_stop_line_vision()
    ...
    def approaching_stop_line_vision(self):
        ...
        if self.stop_line_distance_median is not None:
            ...
            dist_to_drive = self.stop_line_distance_median - \
    ...
    def tracking_local_path(self):
        # var1=initial distance from stop_line, #var2=path to follow
        ...
        if self.curr_state.just_switched:
            ...
            if self.conditions[nac.TRUST_GPS] and not \
                ...
                if USE_ADVANCED_NETWORK_FOR_STOPLINES:
                    ...
                    e2, _, _ = self.detect.detect_lane(self.car.frame,
                                                       SHOW_IMGS)
                ...
                if self.stop_line_distance_median is not None:
                    ...
                    d = self.stop_line_distance_median - \
                        self.car.encoder_distance
        ...
        if idx_point_ahead >= max_idx:  # we reached the end of the path
            ...
            out_speed, out_angle = self.\
                controller.get_control(e2, yaw_error, 0.0,
                                       self.desired_speed, gains=gains)
            self.car.drive(out_speed, np.rad2deg(out_angle))

    def roundabout_navigation(self):                            ## not really usefull
        self.switch_to_state(nac.TRACKING_LOCAL_PATH)
    ...
    def waiting_for_pedestrian(self):                           
        ...
        if dist_ahead < OBSTACLE_DISTANCE_THRESHOLD:
            ...
            self.activate_routines([])
            self.curr_state.var1 = last_seen_pedestrian_time = time()       ## Why this line exist ?
        else:
    ...
    def overtaking_static_car(self):
        ...
        if sub_state == OT_SWITCHING_LANE:
            if just_sub_switched:
                self.car.drive_angle(-OVERTAKE_STEER_ANGLE)                 ## Do we need to check for obstacles first?
    ...
    def tailing_car(self):
        ...
        if dist > OBSTACLE_DISTANCE_THRESHOLD+0.05:
            ...
            if self.conditions[nac.CAN_OVERTAKE]:
                if self.conditions[nac.HIGHWAY]:                        ## is moving car only on highway?
    ...
    def parking(self):
        ...
        # T parking manouver
                            ...
                            nac.T_STEP4,
                            nac.T_STEP5]:
            self.parking_t(just_changed, park_state, park_state)
        ...
        # S parking manouver
                            ...
                            nac.S_STEP6,
                            nac.S_STEP7]:
            self.parking_s(just_changed, park_state, park_state)
    ...
    def parking_localizing(self, just_changed, park_state, park_type):
        ...
        if (self.parking_method == 'gps' or ALWAYS_USE_GPS_FOR_PARKING) \
            ...
            if just_changed:
                ...
                self.car.drive_speed(PARK_SEARCH_SPEED)
                self.curr_state.var1 = (park_state, park_type, False)       ## already updated few lines before, is there any reason we need to reupdate??
    ...
    def parking_st0(self, just_changed, park_state, park_type):
        ...
        if just_changed:
            # TODO change the behaviour: go to next event or something      ## simply go back to lane_following ?
    ...
    def parking_s5(self, just_changed, park_state, park_type):
        ...
        if self.car.dist_loc > DIST_4S:
            self.car.drive_speed(0.0)
            sleep(SLEEP_AFTER_STOPPING)
            print('Parked')                                     ## Forget to delete? We accomplish parking in step 4
    ...
    def classifying_obstacle(self):
        ...
        if OBSTACLE_DISTANCE_THRESHOLD <= dist:  #
            ...
            print('Capturing imgs')                             ## Where are we capturing images?
        else:
    ...
    def follow_lane(self):
        ...
        speed, angle_ref = self.controller.get_control(e2, e3, 0,
                                                       self.desired_speed)          ## speed seems not used
    ...
    def detect_stop_line(self):
        ...
        if USE_ADVANCED_NETWORK_FOR_STOPLINES:
            stopline_x, _, _ = self.detect.detect_stop_line2(
                    self.car.frame, show_ROI=SHOW_IMGS)
            dist = stopline_x + 0.05
        else:
            dist = self.detect.detect_stop_line(self.car.frame,
                                                show_ROI=SHOW_IMGS)

        past_detections = self.routines[nac.DETECT_STOP_LINE].var2
        # -0.1 #network is more accurate in this range
        if dist < STOP_LINE_APPROACH_DISTANCE-0.05:
            DETECTION_DEQUE_LENGTH = 50                             ## move to nac?
            SAMPLE_BEFORE_CONFIDENCE = 20                           ## move to nac?
            # var1 holds last detection time
            if self.routines[nac.DETECT_STOP_LINE].var1 is not None:
                last_detection_time = self.routines[nac.DETECT_STOP_LINE].var1
            else:
            ...
            if curr_time - last_detection_time > 0.5:
                ...
                self.routines[nac.DETECT_STOP_LINE].var2 = past_detections = \
            ...
            if len(past_detections) > SAMPLE_BEFORE_CONFIDENCE:
                ...
                self.stop_line_distance_median = np.median(past_detections)
            else:
                self.stop_line_distance_median = None
            self.routines[nac.DETECT_STOP_LINE].var1 = curr_time
        else:
            self.stop_line_distance_median = None
                ...
                if len(idx_close_signs) > 0:
                    for i in idx_close_signs:
                        if self.sign_seen[i] == 0:
                            self.sign_seen[i] = 1
                            print(f'SEEN SIGN \
                                    {nac.SIGN_NAMES[self.sign_types[i]]}, \
                                    at pos {self.sign_points[i]}')
                            self.curr_sign = nac.SIGN_NAMES[self.sign_types[i]]
                else:
                    self.curr_sign = nac.NO_SIGN

            else:  # Use signs                                  ## error: code is unreachable ???
    ...
    def update_state(self):
        ...
        if self.conditions[nac.TRUST_GPS]:
            ...
            closest_node, distance = self.\
                path_planner.get_closest_node(est_pos)              ## distance seems not be used 
    ...
    def switch_to_prev_state(self):             ## used only once, could be handeled differently
    ...
    # Utility functions
    def get_frames_in_range(self, start_dist, end_dist=0.0):                ## should be moved to help functions?
