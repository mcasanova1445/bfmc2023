...
class AutomobileDataSimulator(Automobile_Data):
    def __init__(self,
        ...
        if trig_control:
            self.pub = rospy.Publisher('/automobile/command',
                                       String,
                                       queue_size=1)
            self.steer_updater = rospy.Timer(
                    rospy.Duration(1/STEER_UPDATE_FREQ),
                    self.steer_update_callback)
            self.drive_dist_updater = rospy.Timer(
                    rospy.Duration(ENCODER_TIMER),
                    self.drive_distance_callback)
        if trig_bno:
            self.sub_imu = rospy.Subscriber('/automobile/IMU',
                                            IMU,
                                            self.imu_callback)
        if trig_enc:
            self.reset_rel_pose()
            # the callback will also do velocity
            rospy.Timer(rospy.Duration(ENCODER_TIMER),
                        self.encoder_distance_callback)
        if trig_sonar:
            self.sub_son = rospy.Subscriber('/automobile/sonar1',
                                            Range,
                                            self.sonar_callback)
            self.sub_lateral_son = rospy.Subscriber(
                    '/automobile/sonar2',
                    Range,
                    self.lateral_sonar_callback)
        if trig_cam:
            self.bridge = CvBridge()
            self.sub_cam = rospy.Subscriber("/automobile/image_raw",
                                            Image,
                                            self.camera_callback)
        if trig_gps:
            self.sub_pos = rospy.Subscriber("/automobile/localisation",
                                            localisation,
    ...
    def lateral_sonar_callback(self, data) -> None:
        ...
        :acts on: self.lateral_sonar_distance,
                  self.filtered_lateral_sonar_distance
        """
