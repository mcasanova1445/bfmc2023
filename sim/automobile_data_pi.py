...
class AutomobileDataPi(Automobile_Data):
    def __init__(self,
        ...
        self.lateral_sonar_distance_buffer = collections.deque(
                maxlen=SONAR_DEQUE_LENGTH)

        self.center_sonar_distance = 3.0
        self.center_sonar_distance_buffer = collections.deque(
                maxlen=SONAR_DEQUE_LENGTH)
        self.filtered_center_sonar_distance = 3.0

        self.left_sonar_distance = 3.0
        self.left_sonar_distance_buffer = collections.deque(
                maxlen=SONAR_DEQUE_LENGTH)
        self.filtered_left_sonar_distance = 3.0

        self.right_sonar_distance = 3.0
        self.right_sonar_distance_buffer = collections.deque(
                maxlen=SONAR_DEQUE_LENGTH)
        self.filtered_right_sonar_distance = 3.0

        self.encoder_velocity_buffer = collections.deque(
                maxlen=SONAR_DEQUE_LENGTH)
        ...
        if trig_control:
            self.pub_speed = rospy.Publisher(
                    '/automobile/command/speed', Float32, queue_size=1)
            self.pub_steer = rospy.Publisher(
                    '/automobile/command/steer', Float32, queue_size=1)
            self.pub_stop = rospy.Publisher(
                    '/automobile/command/stop', Float32, queue_size=1)
            self.pub_position = rospy.Publisher(
                    '/automobile/command/position', Float32, queue_size=1)
            self.sub_position = rospy.Subscriber(
                    "/automobile/feedback/position", Bool,
                    self.feedback_position_callback)
        if trig_bno:
            self.sub_imu = rospy.Subscriber(
                    '/automobile/imu', IMU, self.imu_callback)
        if trig_enc:
            self.sub_encSpeed = rospy.Subscriber(
                    '/automobile/encoder/speed', Float32,
                    self.encoder_velocity_callback)
            self.sub_encDist = rospy.Subscriber(
                    '/automobile/encoder/distance', Float32,
                    self.encoder_distance_callback)
            self.reset_rel_pose()
        if trig_sonar:
            self.sub_son_ahead_center = rospy.Subscriber(
                    '/automobile/sonar/ahead/center', Float32,
                    self.center_sonar_callback)
            self.sub_son_ahead_left = rospy.Subscriber(
                    '/automobile/sonar/ahead/left', Float32,
                    self.left_sonar_callback)
            self.sub_son_ahead_right = rospy.Subscriber(
                    '/automobile/sonar/ahead/right', Float32,
                    self.right_sonar_callback)

            self.sub_lateral = rospy.Subscriber(
                    '/automobile/sonar/lateral', Float32,
                    self.lateral_sonar_callback)
        if trig_cam:
            raise NotImplementedError("cam not implemented yet")
        if trig_gps:
            self.sub_pos = rospy.Subscriber(
                    "/automobile/localisation", localisation,
    ...
    def center_sonar_callback(self, data) -> None:
        ...
        """
        self.center_sonar_distance = data.data \
            if data.data > 0 else self.center_sonar_distance
        self.center_sonar_distance_buffer.append(self.center_sonar_distance)
        self.filtered_center_sonar_distance = np.median(
                self.center_sonar_distance_buffer)
    ...
    def left_sonar_callback(self, data) -> None:
        ...
        :acts on: self.sonar_distance, self.filtered_sonar_distance
        """
        self.left_sonar_distance = data.data \
            if data.data > 0 else self.left_sonar_distance
        self.left_sonar_distance_buffer.append(self.left_sonar_distance)
        self.filtered_left_sonar_distance = np.median(
                self.left_sonar_distance_buffer)
        self.update_sonar_distance()
    ...
    def right_sonar_callback(self, data) -> None:
        ...
        self.right_sonar_distance = data.data \
            if data.data > 0 else self.right_sonar_distance
        self.right_sonar_distance_buffer.append(self.right_sonar_distance)
        self.filtered_right_sonar_distance = np.median(
                self.right_sonar_distance_buffer)
        self.update_sonar_distance()

    def update_sonar_distance(self):
        if self.steer < -SONAR_THRESHOLD:
            self.sonar_distance = min(self.left_sonar_distance,
                                      self.center_sonar_distance)
            self.filtered_sonar_distance = min(
                    self.filtered_left_sonar_distance,
            ...
            self.sonar_distance = min(self.right_sonar_distance,
                                      self.center_sonar_distance)
            self.filtered_sonar_distance = min(
    ...
    def lateral_sonar_callback(self, data) -> None:
        ...
        :acts on: self.sonar_distance, self.filtered_sonar_distance
        """
        self.lateral_sonar_distance = data.data \
            if data.data > 0 else self.lateral_sonar_distance
        self.lateral_sonar_distance_buffer.append(self.lateral_sonar_distance)
        self.filtered_lateral_sonar_distance = np.median(
                self.lateral_sonar_distance_buffer)
    ...
    def encoder_velocity_callback(self, data) -> None:
        ...
        self.filtered_encoder_velocity = np.median(
                self.encoder_velocity_buffer)
