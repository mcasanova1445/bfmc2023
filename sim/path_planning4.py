...
class PathPlanning():
    def __init__(self, map_img):
        ...
        # state=following/intersection/roundabout,
        # next_action=left/right/straight, path_params_ahead)
        ...
        # self.intersection_cen = ['347', '37', '39', '38', '11', '29', '30',
        #                          '28', '371', '84', '9', '20', '20', '82',
        #                          '75', '74', '83', '312', '315', '65',
        #                          '468', '10', '64', '57', '56', '66', '73',
        #                          '424', '48', '47', '46']
        self.intersection_cen = ['468', '12', '37', '39', '38', '11', '29',
                                 '30', '28', '84', '9', '19', '20', '21',
                                 '82', '75', '74', '83', '312', '315', '65',
                                 '10', '64', '57', '56', '55', '66', '73',
                                 '48', '47', '46']

        # define intersecion entrance nodes
        self.intersection_in = [77, 45, 54, 50, 41, 79, 374, 52, 43, 81, 36,
                                4, 68, 2, 34, 70, 6, 32, 72, 59, 15, 16, 27,
                                14, 25, 18, 61, 23, 63]
        self.intersection_in = [str(i) for i in self.intersection_in]

        # define intersecion exit nodes
        self.intersection_out = [76, 78, 80, 40, 42, 44, 49, 104, 53, 67, 69,
                                 71, 1, 3, 5, 7, 8, 31, 33, 35, 22, 24, 26,
                                 13, 17, 58, 60, 62]
        self.intersection_out = [str(i) for i in self.intersection_out]

        # define left turns tuples
        # self.left_turns = [(45,42),(43,40),(54,51),(36,33),(34,31),(6,3),
        #                    (2,7),(2,8),(4,1),(70,67),(72,69),(27,24),(25,22),
        #                    (16,13),(61,58),(63,60)]

        # define crosswalks
        # note: parking spots are included too (177,162)
        self.crosswalk = [92, 96, 276, 295, 170, 266, 177, 162]
        ...
        self.forbidden_nodes = self.intersection_cen + self.intersection_in + \
            self.intersection_out + self.crosswalk + self.ra + \
            self.ra_enter + self.ra_exit + self.junctions

        self.no_yaw_calibration_nodes = [str(i) for i in
                                         [340, 341, 342, 464, 465, 466, 467]]

        # self.near_crosswalk_nodes = [str(i) for i in
        #                              [67,95,80,92,96,295,296,276,277,]]

        # event points
        # created in R coord
        self.event_points = np.load('data/event_points.npy')
        self.event_types = np.load('data/event_types.npy')
        assert len(self.event_points) == len(self.event_types), \
            "event points and types are not the same length"
        ...
        self.all_nodes_coords = np.array([self.get_coord(node) for node in
                                          self.all_start_nodes])

        # highway nodes
        self.highway_nodes = [str(i) for i in [*range(311, 338),
                                               *range(375, 398),
                                               *range(348, 371),
                                               *range(400, 424)]]
            ...
            if curr_node in self.ra_enter:
                if not (prev_node in self.ra):
                    ...
                    self.route_list.append(
                            (xc, yc, np.rad2deg(np.arctan2(dy, dx))))
                else:
                    if curr_node == '302':
                        continue
                    else:
    ...
    def compute_route_list(self):
        ...
        while curr_node != self.target:
            ...
            if next_is_roundabout_enter:
                if curr_node == "342":
                    self.route_list.append(
                            (xc, yc, np.rad2deg(np.arctan2(-1, 0))))
                    dx = xc - xp
                    dy = yc - yp
                    self.route_list.append(
                            (xc, yc+0.3*dy, np.rad2deg(np.arctan2(-1, 0))))
                else:
                    dx = xc-xp
                    dy = yc-yp
                    self.route_list.append(
                    ...
                    dy = yc - yp
                    self.route_list.append(
                            (xc+0.3*dx, yc+0.3*dy, np.rad2deg(
                                np.arctan2(dy, dx))))
                # enter the roundabout
                self.navigator.append("enter roundabout at " + curr_node)
                prev_node, curr_node, next_node = self.roundabout_navigation(
                ...
                dy = yc - yp
                self.route_list.append(
                        (xc+0.3*dx, yc+0.3*dy, np.rad2deg(np.arctan2(dy, dx))))
                # enter the intersection
                self.navigator.append("enter intersection at " + curr_node)
                prev_node, curr_node, next_node = self.intersection_navigation(
                        prev_node, curr_node, next_node)
                continue
    ...
    def compute_shortest_path(self, source=None,
        ...
        for i in range(len(route_nx)-1):
            self.route_graph.add_edges_from(
                    [(route_nx[i], route_nx[i+1],
                      self.G.get_edge_data(route_nx[i], route_nx[i+1]))])
    ...
    def augment_path(self, draw=True):
        ...
        if len(path_event_points) > 0:
            ...
            for i in range(len(path_event_points)):
                ...
                self.path_event_points_idx.append(
                        path_event_points_idx[min_idx])
                self.path_event_points_distances.append(
                        0.01*path_event_points_idx[min_idx])
        ...
        for i in range(len(path_event_points)):
            ...
            if t.startswith('intersection') or t.startswith('roundabout'):
                ...
                if draw:
                    for p in path_ahead:
                        cv.circle(self.map, hf.mR2pix(p), 10, (200, 150, 0), 5)
            elif t.startswith('junction') or t.startswith('highway'):
                assert len(self.path) > 0
                path_ahead = self.path[
                        self.path_event_points_idx[i]:min(
                            self.path_event_points_idx[i]+140, len(self.path))]
                path_event_path_ahead.append(path_ahead)
                if draw:
                    for p in path_ahead:
                        cv.circle(self.map, hf.mR2pix(p), 10, (200, 150, 0), 5)
    ...
    def generate_path_passing_through(self, list_of_nodes, step_length=0.01):
        """
        Extend the path generation from source-target to a sequence of
        nodes/locations
        """
        assert len(list_of_nodes) >= 2, \
            "List of nodes must have at least 2 nodes"
        print("Generating path passing through: ", list_of_nodes)
        src = list_of_nodes[0]
        tgt = list_of_nodes[1]
        complete_path = self.compute_shortest_path(source=src,
                                                   target=tgt,
                                                   step_length=step_length)
        for i in range(1, len(list_of_nodes)-1):
            src = list_of_nodes[i]
            tgt = list_of_nodes[i+1]
            self.compute_shortest_path(source=src,
                                       target=tgt,
                                       step_length=step_length)
    ...
    def get_path_ahead(self, index, look_ahead=100):
        ...
        return np.array(self.path[index:min(index + look_ahead,
                                            len(self.path)-1), :])
    ...
    def get_closest_stop_line(self, nx, ny, draw=False):
        ...
        """
        index_closest = np.argmin(np.hypot(nx - self.stop_points[:, 0],
                                           ny - self.stop_points[:, 1]))
        print(f'Closest stop point is {self.stop_points[index_closest, :]}, \
                Point is {nx, ny}')
        # draw a circle around the closest stop point
        if draw:
            cv.circle(self.map,
                      hf.mR2pix(self.stop_points[index_closest]),
                      8, (0, 255, 0), 4)
        return self.stop_points[index_closest, 0], \
            self.stop_points[index_closest, 1]
    ...
    def interpolate_route(route, step_length):
        ...
        for i, (x, y) in enumerate(zip(path_X, path_Y)):
            if not (np.isclose(x, prev_x, rtol=1e-5) and
                    np.isclose(y, prev_y, rtol=1e-5)):
                path.append([x, y])
            # else:
    ...
    def draw_path(self):
        ...
        # draw trajectory
        cv.polylines(self.map, [hf.mR2pix(self.path)], False, (200, 200, 0),
