...
ROUNDABOUT_NAVIGATOR_IN = "models/round_in.onnx"
ROUNDABOUT_NAVIGATOR_ABOUT = "models/round_about8001.onnx"
ROUNDABOUT_NAVIGATOR_OUT = "models/round_out.onnx"
...
class Detection:
    ...
    def __init__(self) -> None:
        ...
        # intersection right
        self.intersection_navigator_right = cv.dnn.\
            readNetFromONNX(INTERSECTION_NAVIGATOR_RIGHT)
        self.intersection_right_cnt = 0
        self.avg_intersection_right_detection_time = 0

        # intersection left
        self.intersection_navigator_left = cv.dnn.\
            readNetFromONNX(INTERSECTION_NAVIGATOR_LEFT)
        self.intersection_left_cnt = 0
        self.avg_intersection_left_detection_time = 0

        # intersection forward
        self.intersection_navigator_forward = cv.dnn.\
            readNetFromONNX(INTERSECTION_NAVIGATOR_FORWARD)
        self.intersection_forward_cnt = 0
        self.avg_intersection_forward_detection_time = 0

        # roundabout in
        self.roundabout_navigator_in = cv.dnn.\
            readNetFromONNX(ROUNDABOUT_NAVIGATOR_IN)
        self.roundabout_in_cnt = 0
        self.avg_roundabout_in_detection_time = 0

        # roundabout about
        self.roundabout_navigator_about = cv.dnn.\
            readNetFromONNX(ROUNDABOUT_NAVIGATOR_ABOUT)
        self.roundabout_about_cnt = 0
        self.avg_roundabout_about_detection_time = 0

        # roundabout out
        self.roundabout_navigator_out = cv.dnn.\
            readNetFromONNX(ROUNDABOUT_NAVIGATOR_OUT)
        self.roundabout_out_cnt = 0
        self.avg_roundabout_out_detection_time = 0

        # stop line detection
        self.stopline_estimator = cv.dnn.readNetFromONNX(
                STOPLINE_ESTIMATOR_PATH)
        self.est_dist_to_stopline = 1.0
        self.avg_stopline_detection_time = 0
        self.stopline_cnt = 0

        # stop line detection advanced
        self.stopline_estimator_adv = cv.dnn.readNetFromONNX(
                STOPLINE_ESTIMATOR_ADV_PATH)
    ...
    def detect_lane(self, frame, show_ROI=True, faster=False):
        ...
        if faster:
            blob = cv.dnn.blobFromImage(images, 1.0, IMG_SIZE, 0, swapRB=True,
                                        crop=False)
        else:
