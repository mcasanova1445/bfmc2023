...
class Detection:
    ...
    def __init__(self) -> None:
        ...
        # stop line detection
        self.stop_line_estimator = cv.dnn.readNetFromONNX(
                STOP_LINE_ESTIMATOR_PATH)
        self.est_dist_to_stop_line = 1.0
        self.avg_stop_line_detection_time = 0
        self.stop_line_cnt = 0

        # stop line detection advanced
        self.stop_line_estimator_adv = cv.dnn.readNetFromONNX(
                STOP_LINE_ESTIMATOR_ADV_PATH)
    ...
    def detect_stop_line(self, frame, show_ROI=True):
        ...
        self.avg_stop_line_detection_time = \
            (self.avg_stop_line_detection_time*self.lane_cnt +
             stop_line_detection_time) / (self.lane_cnt+1)
        self.lane_cnt += 1
        if show_ROI:
